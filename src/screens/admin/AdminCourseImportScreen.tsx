...
import { parseCourseImportJson, type CourseImportV1 } from '../../utils/courseImport';
...
export default function AdminCourseImportScreen() {
  ...
  const parsed = useMemo(() => jsonText.trim() ? parseCourseImportJson(jsonText) : null, [jsonText]);
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Import Course JSON</Text>
        <View style={styles.backButton} />
      </View>

      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <Text style={styles.sectionLabel}>GolfCaddie v1 JSON</Text>
        <TextInput
          style={styles.jsonInput}
          value={jsonText}
          onChangeText={value => { setJsonText(value); setNearbyOverride(false); }}
          placeholder="Paste golfcaddie.course.v1 JSON here…"
        ...
        {!parsed && (
          <View style={styles.notice}>
            <Text style={styles.noticeText}>Nothing is written to Supabase until the JSON parses and passes validation.</Text>
          </View>
        )}
          ...
          <>
            ...
            {errors.length > 0 && (
              <>
                ...
                <View style={styles.issueCard}>
                  {errors.map((issue, index) => <Issue key={`${issue.path}-${index}`} path={issue.path} text={issue.message} bad />)}
                </View>
              </>
            )}
            ...
            {warnings.length > 0 && (
              <>
                ...
                <View style={styles.issueCard}>
                  {warnings.map((issue, index) => <Issue key={`${issue.path}-${index}`} path={issue.path} text={issue.message} />)}
                </View>
              </>
            )}
            ...
            {data && (
              <>
                ...
                <TouchableOpacity
                  style={[styles.importButton, saving && styles.disabled]}
                  onPress={() => void importCourse()}
                  disabled={saving}
                >
...
function Count({ label, value, bad = false }: { label: string; value: number; bad?: boolean }) {
  return (
    <View style={styles.count}>
      <Text style={[styles.countValue, bad && styles.badText]}>{value}</Text>
...
function Issue({ path, text, bad = false }: { path: string; text: string; bad?: boolean }) {
  return (
    <View style={styles.issueRow}>
      <View style={[styles.issueDot, bad ? styles.errorDot : styles.warningDot]} />
      <View style={{ flex: 1 }}>
        <Text style={styles.issuePath}>{path}</Text>
...
function PreviewRow({ label, value, last = false }: { label: string; value: string; last?: boolean }) {
  return (
    <View style={[styles.previewRow, !last && styles.divider]}>
...
const styles = StyleSheet.create({
  ...
  jsonInput: { minHeight: 220, padding: Spacing.base, borderRadius: Radius.lg, borderWidth: 1, borderColor: Colors.border, backgroundColor: Colors.surface1, color: Colors.text, fontFamily: Font.regular, fontSize: FontSize.sm },
