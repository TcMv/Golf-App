...
export default function AdminOsmMappingScreen() {
  ...
  const counts = useMemo(() => {
    ...
    for (const suggestion of result?.batch.suggestions ?? []) {
      output[suggestion.feature_type] = (output[suggestion.feature_type] ?? 0) + 1;
    }
    ...
    try {
      ...
      const response = await fetch(OSM_PUBLIC_OVERPASS_ENDPOINT, {
        ...
        headers: {
          ...
          'User-Agent': 'GolfCaddie/1.0 course-mapping-admin',
        },
  ...
  const queue = async () => {
    ...
    try {
      ...
      Alert.alert(
        'OSM suggestions queued',
        `${result.batch.suggestions.length} features were added to the human review queue for ${selectedCourse.name}. No playable geometry was changed.`,
        [
          { text: 'Stay here' },
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right']}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1, alignItems: 'center' }}><Text style={styles.headerTitle}>Generate from OSM</Text><Text style={styles.subtitle}>First automated mapping source</Text></View>
        <View style={styles.backButton} />
      </View>

      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.notice}>
          <Text style={styles.noticeTitle}>Commercially reusable source, human-reviewed output</Text>
          <Text style={styles.noticeText}>OpenStreetMap data is ODbL licensed. This prototype uses the public Overpass service to generate pending suggestions only. Production-scale commercial use should move to a paid or self-hosted Overpass/data pipeline.</Text>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipRow}>
          {courses.map(course => {
            ...
            return (
              <TouchableOpacity key={course.id} style={[styles.chip, active && styles.chipActive]} onPress={() => { setSelectedCourseId(course.id); setResult(null); }}>
                <Text style={[styles.chipText, active && styles.chipTextActive]}>{course.name}</Text>
              </TouchableOpacity>
            );
          })}
        </ScrollView>
        ...
        {selectedCourse && (
          <View style={styles.courseCard}>
            ...
            <Text style={styles.courseMeta}>{selectedCourse.holes} holes · centre {selectedCourse.lat?.toFixed(5) ?? 'missing'}, {selectedCourse.lng?.toFixed(5) ?? 'missing'}</Text>
            <Text style={styles.courseMeta}>Search radius: 1.8 km</Text>
          </View>
        )}

        <TouchableOpacity style={[styles.generateButton, fetching && styles.disabled]} onPress={() => void generate()} disabled={fetching || !selectedCourse}>
          ...
          <>
            ...
            <View style={styles.summaryCard}>
              ...
              <View style={styles.metrics}>
                <Metric label="Direct refs" value={result.directlyAssigned} />
                <Metric label="Inferred" value={result.inferredAssignments} />
                <Metric label="Skipped" value={result.skipped} />
              </View>
              {counts.map(([feature, count]) => (
                <View key={feature} style={styles.countLine}><Text style={styles.countFeature}>{feature.replaceAll('_', ' ')}</Text><Text style={styles.countValue}>{count}</Text></View>
              ))}
            </View>

            {result.issues.length > 0 && (
              <View style={styles.issueCard}>
                {result.issues.slice(0, 12).map((issue, index) => <Text key={index} style={issue.severity === 'warning' ? styles.warningText : styles.infoText}>{issue.severity === 'warning' ? '⚠' : '•'} {issue.message}</Text>)}
                {result.issues.length > 12 && <Text style={styles.infoText}>…and {result.issues.length - 12} more mapping notes.</Text>}
              </View>
            )}
